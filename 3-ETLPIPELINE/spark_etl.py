...
"""
Script that extracts and process information by using spark dataframes from mysql and dynamo Shopping DBs to store it into Redshift DWH
"""
...
SPARK_CONNECTION = os.getenv('SPARK_CONNECTION')
...
findspark.init(SPARK_CONNECTION)
sc = SparkContext(appName="shopping")
...
def get_mysql_query_df(query):
    """
    Get mysql query results as spark dataframe

    :param query: Query to be runt and retrieved as spark dataframe
    ...
    return sqlContext.createDataFrame(pd.read_sql(query, con=db_connection))


def get_dynamo_query_df(table_name, conditionals):
    """
    Get dynamo query results as spark dataframe
    ...
    return sqlContext.createDataFrame(pd.json_normalize(json.loads(json.dumps(table.query(KeyConditionExpression=conditionals)['Items'], cls=DecimalEncoder))))


def send_dataframe_to_redshift(table_name, dataframe):
    """
    Send spark dataframe to redshift table
...
# Get mysql and dynamo data as spark dataframe
...
mysql_df = mysql_df.withColumn('zip_code', udf(lambda address: re.search("CP (\d+)", address).group(1))(mysql_df.address))
mysql_df = mysql_df.withColumn('total_price', udf(lambda price: price * MXN_TO_USD_RATE)(mysql_df.total_price))
mysql_df = mysql_df.drop('address')

# Processing dynamo_df to have clean data for the DWH (extract timestamp and rename columns for DWH column names)
dynamo_df = dynamo_df.withColumn('order_timestamp', udf(lambda x: x.split('_')[1])(dynamo_df.customerid_timestamp))

dynamo_df = dynamo_df.withColumnRenamed('itemsku', 'itemid_sku') \
    .withColumnRenamed('quantity_bought', 'quantity') \
    .withColumnRenamed('item.title', 'item_name') \
    .withColumnRenamed('customer.zipcode', 'zip_code')

for column in ['customerid_timestamp', 'order_date', 'customerid', 'customer.firstname', 'customer.lastname', 'item.price']:
    dynamo_df = dynamo_df.drop(column)

# Making a union of clean mysql_df and clean dynamo_df data for DWH
redshift_df = mysql_df.unionByName(dynamo_df)
